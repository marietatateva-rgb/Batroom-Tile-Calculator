...
export function createClientsPage() {
    ...
    repo.getAllInvoices().forEach((invoice) => {
        const clientId = invoice.recipient.id || 'unknown';
      ...
      <div class="page-content">
        ...
        return `
                  <tr>
                    ...
                    <td>${isCompany ? client.taxId || client.id || '-' : client.nationalIdNumber || '-'}</td>
                    <td>${isCompany ? client.address || '-' : '-'}</td>
                    <td>${invoiceCount}</td>
                    <td>
    ...
    page.querySelectorAll('[data-action]').forEach((btn) => {
        btn.addEventListener('click', (e) => {
            ...
            switch (action) {
                case 'view':
                    if (clientId)
                        navigateTo(`/clients/${clientId}/view`);
                    break;
                case 'edit':
                    if (clientId)
...
export function createClientViewPage(id) {
    const page = document.createElement('div');
...
export function createClientEditPage(id) {
    const page = document.createElement('div');
          ...
          <form class="client-form" data-client-id="${id}" data-is-company="${isCompany}">
            ...
            ${isCompany ? `
              <div class="form-group">
                ...
                <input type="text" id="taxId" name="taxId" value="${client.taxId || ''}">
              </div>
              
              <div class="form-group">
                <label for="companyId">Company ID</label>
                <input type="text" id="companyId" name="companyId" value="${client.id || ''}">
              </div>
              
              <div class="form-group">
                <label for="manager">Manager</label>
                <input type="text" id="manager" name="manager" value="${client.manager || ''}">
              </div>
            ...
            ` : `
              <div class="form-group">
                ...
                <input type="text" id="nationalId" name="nationalId" value="${client.nationalIdNumber || ''}">
              </div>
            `}
    ...
    `;
        ...
        const form = page.querySelector('.client-form');
        ...
        form?.addEventListener('submit', (e) => {
            ...
            if (type === 'company') {
                updates.taxId = (formData.get('taxId') || '');
                updates.id = (formData.get('companyId') || '');
                updates.manager = (formData.get('manager') || '');
                updates.address = (formData.get('address') || '');
                // Remove individual fields
                if ('nationalIdNumber' in updates)
                    delete updates.nationalIdNumber;
            }
            else {
                updates.nationalIdNumber = (formData.get('nationalId') || '');
...
export function createAddClientPage() {
    const page = document.createElement('div');
  ...
  `;
    ...
    const typeSelect = page.querySelector('#type');
    const companyFields = page.querySelector('#company-fields');
    const individualFields = page.querySelector('#individual-fields');
    const form = page.querySelector('.client-form');
    typeSelect?.addEventListener('change', () => {
    ...
    form?.addEventListener('submit', (e) => {
        ...
        if (type === 'company') {
            ...
            repo.addRecipient({
                name: (formData.get('name') || ''),
                nationalIdNumber: (formData.get('nationalId') || ''),
            });
        }
