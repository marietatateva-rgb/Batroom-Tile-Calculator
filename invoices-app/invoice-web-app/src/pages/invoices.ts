...
export function createInvoiceViewPage(id: string): HTMLElement {
  const page = document.createElement('div')
  page.className = 'page page-invoice-view'
  
  const repo = getGlobalRepo()
  const invoice = (repo as any).findInvoiceById?.(id)
        ...
        <div class="invoice-section">
          ...
          <div class="invoice-details">
            ${recipient ? `
              ...
              ${(recipient as any).id ? `<p><span class="label">Company ID:</span> <span class="value">${(recipient as any).id}</span></p>` : ''}
              ${(recipient as any).taxId ? `<p><span class="label">Tax ID:</span> <span class="value">${(recipient as any).taxId}</span></p>` : ''}
              ${(recipient as any).address ? `<p><span class="label">Address:</span> <span class="value">${(recipient as any).address}</span></p>` : ''}
              ${(recipient as any).manager ? `<p><span class="label">Manager:</span> <span class="value">${(recipient as any).manager}</span></p>` : ''}
...
export function createInvoiceEditPage(id: string): HTMLElement {
  const page = document.createElement('div')
  ...
  const form = page.querySelector('.invoice-edit-form') as HTMLFormElement
...
export function createInvoicePdfPage(id: string): HTMLElement {
  const page = document.createElement('div')
    ...
    <div style="font-family: Arial, sans-serif; max-width: 900px; margin: 0 auto; padding: 40px; background: white;">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 40px;">
        ...
        <div>
          ...
          ${(recipient as any).id ? `<p style="margin: 5px 0; color: #6b7280; font-size: 12px;">ID: ${(recipient as any).id}</p>` : ''}
          ${(recipient as any).taxId ? `<p style="margin: 5px 0; color: #6b7280; font-size: 12px;">Tax ID: ${(recipient as any).taxId}</p>` : ''}
          ${(recipient as any).manager ? `<p style="margin: 5px 0; color: #6b7280; font-size: 12px;">Manager: ${(recipient as any).manager}</p>` : ''}
          ${(recipient as any).address ? `<p style="margin: 5px 0; color: #6b7280; font-size: 12px;">${(recipient as any).address}</p>` : ''}
          ${(recipient as any).nationalIdNumber ? `<p style="margin: 5px 0; color: #6b7280; font-size: 12px;">National ID: ${(recipient as any).nationalIdNumber}</p>` : ''}
        </div>
      </div>
  ...
  const downloadBtn = page.querySelector('#download-pdf-btn') as HTMLButtonElement
  if (downloadBtn) {
    downloadBtn.addEventListener('click', () => {
      const element = page.querySelector('#pdf-container') as HTMLElement
      if (element) {
